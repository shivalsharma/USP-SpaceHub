...
public void suggestAlternative(Long id) {
    ...
    List<AlternativeOption> options =
            alternativeService.findAlternatives(b, 1);
    ...
    AlternativeOption option = options.getFirst();
    ...
    suggestion.setExplanation(option.explanation());
    ...
    notifications.notify(
            ...
            "Suggested: " + option.room().getCode()
                    + " on " + option.date()
                    + " " + option.startTime()
                    + "-" + option.endTime() + ".",
            "ADMIN-ALTERNATIVE-" + b.getId()
                    + "-" + suggestion.getId()
    );
